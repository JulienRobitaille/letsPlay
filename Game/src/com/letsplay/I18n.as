/**
 * Created by julien on 1/24/15.
 */
package com.letsplay {
public class I18n {
    public static const TITLE:String = "LETS PLAY! ;)";
    public static var INTRO:String = "Aujourd'hui 16h15, Timmy et Jocelyne se lance\n dans l'avant première de la pièce:\n Des grenouilles et des chats.\n Timmy n'est pas prêt, il ne se souvient pas de son texte !\n Aidez-le à bien performer !";
    public static var JOUER:String = "Jouer";
    public static var CREDIT:String = "Credit";
    public static var CREDITTEXT:String = "MONTREAL GAME JAM 2015\n\n UN GROS MERCI À L'ETS !\n\nLETS PLAY! ;)\nVous est présenté par\n\nMyriam Thouin\nDamon Péron Laurin\nJulien Robitaille\n Merci à toute l'équipe de son !";

    public static var QUESTION1:String = "Est ce que tu préfère les chiens ou les chats?";
        public static var CHOICE1:String = "Les chiens";
        public static var CHOICE2:String = "Les chats";
        public static var CHOICE3:String = "... (psss Jocelyne? C'est quoi déjà mon texte?)";

    public static var QUESTION2:String = "Les chiens son bruiant, bruns et ils puent!\nJ'aimerais avoir un chat, mais maman ne veut pas.";
        public static var CHOICE4:String = "Brun? Ce n'est même pas vrai! (Mais qu'est-ce que je fait ici?)";
        public static var CHOICE5:String = "Oh ! Regarde le jolie chaton!";
        public static var CHOICE6:String = "... (Oups j'ai un blanc; ma soeur ne sera vraiment pas contente.)";

    public static var QUESTION3:String = "... Timmy aime-tu les chats? (Il n'a vraiment aucune mémoire!)";
        public static var CHOICE7:String = "Oui beaucoup.";
        public static var CHOICE8:String = "Vraiment pas.";
        public static var CHOICE9:String = "... (Qu'est-ce que je fait)";

    public static var QUESTION4:String = "Oh regarde le chaton. Attrapons-le! Aide moi! Aide moi!";
        public static var CHOICE10:String = "Je vais à droite tu vas à gauche!";
        public static var CHOICE11:String = "Ce n'ais même pas un vrai chat. Mathieu la fabriqué pour la pièce.";
        public static var CHOICE12:String = "...";

    public static var QUESTION5:String = "Ohhhh. Il est si beau! Attrapons-le. Ramenons le chez moi. Dis rien à maman.";
        public static var CHOICE13:String = "Il a l'aire sale et méchant. Ce n'est pas une bonne idée.";
        public static var CHOICE14:String = "Seulement si je peux le flatter en premier.";
        public static var CHOICE15:String = "... (Je pers vraiment mon temps. Pourquoi une grenouille adopterait un chat?)";

    public static var QUESTION6:String = "Le chat s'enfuit !!! Vite Vite ! Suivons-le.";
        public static var CHOICE16:String = "... (Je crois que je n'ai pas vraiment le choix.)";

    public static var QUESTION7:String = "Tu l'as laisser s'enfuire ! C'est de ta faute !!! (Elle court dans le parc)";
        public static var CHOICE17:String = "... (Se met à suivre Jocelyne.)";
    public function I18n() {
    }
}
}
